...
function cmount() {
	...
	if ! cryptsetup isLuks $DEV 2>/dev/null; then
		...
		cryptsetup resize $CDEV --size 244189323
	fi
...
function c_usr_verify() {
	...
	diff -u $TESTDIR/checksums-$1.ref $TESTDIR/checksums-$1.tst 2>&1 > /dev/null || {
...
do
	if [ "$i" = "null" ] ; then
		...
		echo "$PASS"|cryptsetup luksFormat --force-password -c $i -s 256 $DEV
	fi
